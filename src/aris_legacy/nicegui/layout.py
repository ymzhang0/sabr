...
from src.aris_legacy.nicegui.themes import THEMES
...
def create_aiida_layout(theme_name="gemini_dark", available_models=None):
            ...
            async def toggle_log():
                ...
                if log_container.visible:
                    ...
                    await ui.run_javascript(
                        f'const log = document.getElementById("c{thought_log.id}"); if(log) log.scrollTop = log.scrollHeight;',
                        respond=False,
                    )
                else:
    ...
    with ui.column().classes("fixed bottom-0 right-0 z-10 p-8 items-center").style(
        "left: 340px; background: linear-gradient(to top, var(--main-bg) 60%, transparent);"
    ):
        with ui.column().classes("w-full max-w-[850px] gap-4"):
            ...
            with ui.row().classes("w-full items-center justify-between"):
                model_select = ui.select(options=available_models, value=available_models[0]).props(
                    ...
                    "text-[12px] font-bold tracking-widest uppercase px-3 py-1 rounded-full cursor-pointer "
                    "text-slate-400 hover:text-primary transition-all w-auto min-w-[120px]"
                )
