...
from src.aris_core.protocols import BaseController


class RemoteAiiDAController(BaseController):
    """
    Complete AiiDA Remote Controller for the archived NiceGUI UI.
    """
            ...
            if role == "user":
                with ui.row().classes("w-full justify-end mb-6"):
                    with ui.column().classes("items-end max-w-[80%]"):
                        ...
                        with (
                            ui.card()
                            .classes("bg-primary/10 p-4 rounded-2xl shadow-none border-none")
    ...
    async def handle_send(self, preset_text=None):
        ...
        try:
            ...
            self._render_terminal(f"Connection Error: {exc}", "ERROR")
        finally:
            ui.run_javascript("window.scrollTo(0, document.body.scrollHeight)")

    async def update_process_status(self):
        archive = self.components["archive_select"].value
        if not archive or archive == "(None)":
            return
        try:
            response = await self.client.get("/v1/aiida/processes")
            if response.status_code == 200:
    ...
    async def switch_context(self, path: str):
        ...
        try:
            response = await self.client.get("/v1/aiida/summary")
            if response.status_code == 200:
                stats = response.json()
            ...
            ui.notify("Failed to fetch DB summary", type="negative")
    ...
    async def handle_node_inspection(self, msg):
        ...
        try:
            response = await self.client.get(f"/v1/aiida/nodes/{pk}")
            if response.status_code == 200:
                self._render_insight(response.json())
        except Exception:  # noqa: BLE001
            pass
        ...
        with self.components["chat_area"]:
            with ui.row().classes("gap-2 py-2 ml-12 mb-8"):
                for suggestion in suggestions:
                    (
                        ui.button(suggestion, on_click=lambda text=suggestion: self.handle_send(text))
    ...
    def _add_to_history_ui(self, path: str):
        ...
        with self.components["archive_history"]:
            (
                ui.item(on_click=lambda: self.switch_context(path))
                .classes("px-8 py-2 rounded-xl hover:bg-white/5 cursor-pointer")
    ...
    async def pick_local_file(self):
        def get_path():
            ...
            selected = filedialog.askopenfilename(filetypes=[("AiiDA Archives", "*.aiida *.zip")])
            root.destroy()
            return selected
